    ...
    mongosh --eval "
        try {
            ...
            rs.initiate({
                _id: 'rs0',
                members: [
                    { _id: 0, host: '$MONGODB_RS_HOST:$MONGODB_DATA_1_EXPOSE_PORT' },
                    { _id: 1, host: '$MONGODB_RS_HOST:$MONGODB_DATA_2_EXPOSE_PORT' },
                    { _id: 2, host: '$MONGODB_RS_HOST:$MONGODB_DATA_3_EXPOSE_PORT' },
                ]
            });
        }
    "
...
[ "$(hostname)" = 'mongodb-data-1' ] && ensure_replica_set_initialized &
